...
const RiskModule: React.FC<Props> = ({ globalFilter }) => {
  ...
  const [vintageCascaderValue, setVintageCascaderValue] = useState<[string, string]>(['creditRange', '0-20万']);
  const [overdueCascaderValue, setOverdueCascaderValue] = useState<[string, string]>(['creditRange', '0-20万']);
  ...
  const vintageDimension = vintageCascaderValue[0] as 'creditRange' | 'pricingRange' | 'repayment';
  const selectedBucket = vintageCascaderValue[1];

  const overdueDimension = overdueCascaderValue[0] as 'creditRange' | 'pricingRange' | 'repayment';
  ...
  const allMonthsVintageData = useMemo(() => {
    return cohorts.map((cohort) => {
      const monthData = getRiskDrilldownVintageData(cohort, vintageDimension);
      const bucketData = monthData.find((item) => item.bucket === selectedBucket);
      const drilldownData = getRiskDrilldownData(cohort, vintageDimension);
  ...
  }, [cohorts, vintageDimension, selectedBucket]);
                  ...
                  <div className="chart-card">
                    <div className="chart-title chart-title-inline-logic" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <Cascader
                        ...
                        onChange={(value) => setVintageCascaderValue(value as [string, string])}
                        placeholder="请选择维度和区间"
                        style={{ minWidth: 200 }}
                        expandTrigger="hover"
                      />
                    </div>
                  ...
                  <div className="chart-card">
                    <div className="chart-title chart-title-inline-logic" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <Cascader
                        ...
                        onChange={(value) => setOverdueCascaderValue(value as [string, string])}
                        placeholder="请选择维度和区间"
                        style={{ minWidth: 200 }}
                        expandTrigger="hover"
                      />
